...
static void XMLCALL XMLstart_read_conf_default(void *data,
	...
	if (strcmp(element_name, "payload") == 0) {
		...
		proto_nodes[proto_nodes_bound - 1].transducers[transducer_i].cal_factor = 1;
		proto_nodes[proto_nodes_bound - 1].transducers[transducer_i].r_to_tval_factor = 1;
		...
		for (i = 0; attr[i]; i += 2) {
			...
			if (strcmp(attr_name, "id") == 0) {
				for (transducer_i = 0; transducer_i < MAX_TRANSDUCERS;
						transducer_i++) {
					if (strcmp(attr_name, nodes[node_i - 1].id) == 0)
						break;
				}
	...
	if (*nodep == MAX_NODES) {
		...
		if (*nodep == nodes_bound) {
			...
			if (i == proto_nodes_bound) {
				fprintf(
						stderr,
						"Error: Packet type %u received from node %u is not listed in the default configuration file\n",
...
void print_usage(char *prog_name) {
	...
	fprintf(
			stdout,
			"Usage: %s <-s SLIPstream server address> <-port SLIPstream server port> <-def default packet configuration file> [-opt optional configuration file] [-u XMPP username -p XMPP password] [-db SAGA db location] [-csv CSV output file] [-verbose]\n",
			prog_name);
#elif defined(USE_SOX)
	fprintf(stdout, "\"%s\" SLIPstream SOX-CSV client\n", prog_name);
	fprintf(
			stdout,
			"Usage: %s <-s SLIPstream server address> <-port SLIPstream server port> <-def default packet configuration file> [-opt optional configuration file] [-u XMPP username -p XMPP password] [-csv CSV output file] [-verbose]\n",
			prog_name);
#elif defined(USE_SAGA)
	fprintf(stdout, "\"%s\" SLIPstream SAGA-CSV client\n", prog_name);
	fprintf(
			stdout,
			"Usage: %s <-s SLIPstream server address> <-port SLIPstream server port> <-def default packet configuration file> [-opt optional configuration file] [-db SAGA db location] [-csv CSV output file] [-verbose]\n",
			prog_name);
#else
	fprintf(stdout, "\"%s\" SLIPstream CSV client\n", prog_name);
	fprintf(
			stdout,
			"Usage: %s <-s SLIPstream server address> <-port SLIPstream server port> <-def default packet configuration file> [-opt optional configuration file] [-csv CSV output file] [-verbose]\n",
			prog_name);
#endif
...
int main(int argc, char *argv[]) {
	...
	sox_conn_t * conn;
#endif
	pthread_t main_loop_thread, saga_writer, slip_ka;
	uint8_t err = 0;
