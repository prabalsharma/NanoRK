...
int main ()
{
  ...
  i2cInit();
//  i2cSend( 0x5e, 0x01, 0, &v );
  i2cSend( 0x5e, 0x7f, 0, &v );
  for(v=0; v<200; v++ ) nrk_spin_wait_us(10000);
...
void tx_task ()
{
  ...
  while (1) {
   ...
   nrk_wait_until_next_period ();
   _nrk_set_next_wakeup(240);
   nrk_int_disable(); 
        ticks=0;
   	_nrk_os_timer_reset();
   	do{
   	} while((volatile)_nrk_os_timer_get()<1);

	TCNT1=0;
   	//_nrk_high_speed_timer_reset(); 
   	do{
	//ticks++;
   	} while((volatile)_nrk_os_timer_get()<4);
   	ticks=TCNT1;
